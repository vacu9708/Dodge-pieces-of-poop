...
public:
	...
	void update_a_piece_of_poop(Sprite& player) {
		double coefficient = (double)score / (10 + (double)score);
		...
		while (true) {
			...
			if (poop_sprite.getGlobalBounds().intersects(player.getGlobalBounds()) == true) {
				score -= 20;
				break;
			}
			if (poop_sprite.getPosition().y > 800 - 50) {
				score++;
				break;
			}
			Sleep(1);
...
public:
	...
	void drop_poop(Sprite& player) {
		...
		while (true) {
			...
			if (falling_interval > 300) // Decrease falling interval
				falling_interval -= 50;
...
int main() {
	...
	while (window.isOpen()) {
		window.draw(player);
		window.draw(sky);
		poop_set.display_score_board(scoreboard, window);
